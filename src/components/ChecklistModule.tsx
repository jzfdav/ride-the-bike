...
export function ChecklistModule() {
	...
	return (
		<motion.section
			...
			className={`relative rounded-2xl p-6 border transition-colors duration-500 ${
				isFullyComplete
		...
		>
			<div className="flex justify-between items-center mb-5">
				<span className="text-[10px] text-oled-gray-400 uppercase tracking-widest font-black flex items-center gap-1.5">
					<ShieldCheck
						className={`w-3.5 h-3.5 ${isFullyComplete ? "text-emerald-500" : "text-pulsar-blue"}`}
					/>{" "}
					Pre-Ride Safety
				</span>
				<div className="flex items-center gap-3">
					<span
						className={`text-[8px] font-black uppercase tracking-[0.2em] ${isFullyComplete ? "text-emerald-500" : "text-white/20"}`}
					>
					...
					>
						...
						<div
							className={`transition-all duration-300 ${
								item.checked ? "text-emerald-500" : "text-white/10"
							}`}
						>
							{item.checked ? (
